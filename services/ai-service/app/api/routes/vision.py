...
@router.post("/ingredients/detections", response_model=ResponseData, response_model_exclude_none=True)
...
@router.post(
    "/ingredients/detections/annotated-image",
    response_model=None,
...
@router.post(
    "/ingredients/detections/with-annotated-image",
    response_model=ResponseData,
    response_model_exclude_none=True,
)
async def detect_ingredients_with_annotated_image(
    file: UploadFile = File(...),
    detection_service: IngredientDetectionService = Depends(get_detection_service),
) -> ResponseData | JSONResponse:
    if not file.content_type or not file.content_type.startswith("image/"):
    ...
    try:
        result = detection_service.detect_with_annotated_image(image_bytes)
        return ResponseData(status=200, message="Success", data=result)
    except IngredientDetectionError as exc:
        return JSONResponse(
            status_code=503,
        ...
        logger.exception("Unexpected error while detecting ingredients with annotated image.")
        return JSONResponse(
            status_code=500,
            content={"status": 500, "message": "Unexpected detection error."},
        )

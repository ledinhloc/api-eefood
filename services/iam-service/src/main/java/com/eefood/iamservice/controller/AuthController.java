...
public class AuthController {
  ...
  public ResponseData<UserResponse> register(@Valid @RequestBody UserSignUpRequest request) {
    ...
    return (userResponse == null)
        ? new ResponseData<>(
            HttpStatus.BAD_REQUEST.value(), ErrorMessage.USER_NOT_FOUND.getMessage())
        : new ResponseData<>(
  ...
  public ResponseData<Void> verifyOtp(@Valid @RequestBody OtpCreateRequest request) {
    boolean verified =
        authService.verifyOtp(request.getEmail(), request.getOtpCode(), request.getOtpType());
    return verified
        ? new ResponseData<>(
            HttpStatus.OK.value(), SuccessMessage.OTP_VERIFIED_SUCCESS.getMessage())
        : new ResponseData<>(
            HttpStatus.BAD_REQUEST.value(), ErrorMessage.OTP_INVALID_OR_EXPIRED.getMessage());
  }
  ...
  public ResponseData<Void> resetPassword(@Valid @RequestBody PasswordResetRequest request) {
    ...
    return new ResponseData<>(
        HttpStatus.OK.value(), SuccessMessage.PASSWORD_RESET_SUCCESS.getMessage());
  }
}

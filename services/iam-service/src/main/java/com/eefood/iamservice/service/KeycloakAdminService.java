...
public class KeycloakAdminService {
  ...
  public void resetPassword(String userId, String newPassword) {
    String token = "Bearer " + getAdminAccessToken();
    Credential payload =
        Credential.builder().type("password").temporary(false).value(newPassword).build();

    ResponseEntity<Void> resp = keycloakClient.resetPassword(token, realm, userId, payload);

    if (resp == null || !resp.getStatusCode().is2xxSuccessful()) {
      log.error(
          "Failed to reset password for userId={} status={}",
          userId,
  ...
  public void assignRealmRoleToUser(String userId, String roleName) {
    ...
    ResponseEntity<Void> resp =
        keycloakClient.addRealmRoleMappingsToUser(token, realm, userId, roles);
    if (resp == null || !resp.getStatusCode().is2xxSuccessful()) {
      log.error(
          "Failed to add realm role={} to user={} status={}",
          roleName,
          userId,
          resp != null ? resp.getStatusCode() : "null response");
      throw ExceptionUtil.badRequest(ErrorMessage.FAIL_UPDATE_ROLE);
  ...
  public void removeRealmRoleFromUser(String userId, String roleName) {
    ...
    ResponseEntity<Void> resp =
        keycloakClient.deleteRealmRoleMappingsFromUser(token, realm, userId, roles);
    if (resp == null || !resp.getStatusCode().is2xxSuccessful()) {
      log.error(
          "Failed to remove realm role={} from user={} status={}",
          roleName,
          userId,
          resp != null ? resp.getStatusCode() : "null response");
      throw ExceptionUtil.badRequest(ErrorMessage.FAIL_UPDATE_ROLE);
